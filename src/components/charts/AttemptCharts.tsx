...
export function AttemptAccuracyChart({ data }: { data: AttemptPoint[] }) {
  return (
    <ChartFrame>
      <BarChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: -12 }} barCategoryGap={8}>
        <CartesianGrid stroke={CHART.grid} vertical={false} />
        <XAxis dataKey="attempt" tickFormatter={(v) => `${v}回目`} {...AXIS_PROPS} />
        <YAxis
          domain={[0, 1]}
        ...
        />
        <Bar dataKey="accuracy" name="正答率" fill={CHART.accuracy} radius={[4, 4, 0, 0]} />
      </BarChart>
    </ChartFrame>
  )
}
