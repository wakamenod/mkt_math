...
export function WeakSetsChart({
  ...
  if (data.length === 0) {
    return <EmptyState title="まだ判定できるデータがありません" hint="何回か解くと出てきます" />
  }
    ...
    <ChartFrame rows={data.length}>
      ...
      >
            ...
            active && payload?.length ? (
              <TooltipBox
                ...
                rows={[
                  ...
                  { label: '解いた回数', value: `${payload[0].payload.attemptCount}回` },
                ]}
              />
            ) : null
          }
        ...
        <Bar
          ...
          onClick={(d) => {
            const id = (d as unknown as { payload?: { exerciseSetId?: string } }).payload?.exerciseSetId
            if (id) onSelect?.(id)
          }}
        >
          <LabelList
