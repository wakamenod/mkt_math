...
export function ExerciseSetPicker({
  ...
  const categories = useMemo(() => {
    const map = new Map<string, { id: string; name: string; sortOrder: number; count: number }>()
    for (const s of sets) {
  ...
  if (sets.length === 0) {
    return <EmptyState title="練習問題が登録されていません" hint="設定画面から追加してください" />
  }
  ...
  return (
    <div>
      ...
      {visible.length === 0 ? (
        <EmptyState title="この大分類にはまだ練習問題がありません" hint="設定画面から追加できます" />
      ) : (
        <div className="grid grid-cols-4 gap-2 sm:grid-cols-6">
          {visible.map((s) => {
            ...
            return (
              ...
              >
                <span className="text-lg font-bold text-slate-900">{s.number}</span>
                <span className="text-[10px] text-slate-400">{s.problem_count}問</span>
                {attempts > 0 && (
