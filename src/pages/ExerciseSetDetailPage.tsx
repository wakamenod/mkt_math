...
import { AttemptAccuracyChart, AttemptPaceChart } from '../components/charts/AttemptCharts'
import { SessionList } from '../components/session/SessionList'
import { Button, Card, EmptyState, ErrorNote, Spinner, StatTile } from '../components/ui'
  ...
  return (
    <>
      <PageTitle
        action={
          canEdit && (
            <Button onClick={() => navigate(`/study?set=${set.id}`)}>この問題を始める</Button>
          )
        }
      >
        {setLabel(set)}
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <StatTile label="通算の正答率" value={formatRate(summary.accuracy)} tone="accuracy" />
        <StatTile label="合計時間" value={formatTotalDuration(summary.totalSeconds)} tone="duration" />
        <StatTile
          label="1問あたり"
      ...
      {attempts.length >= 2 && first && latest && (
        <div className="mt-3 rounded-2xl bg-white p-3 text-sm shadow-sm ring-1 ring-slate-200/70">
          初回 <strong className="tnum">{formatRate(first.accuracy)}</strong> → 最新{' '}
          <strong className="tnum">{formatRate(latest.accuracy)}</strong>
          <span
      ...
      <div className="mt-4 space-y-4">
        {attempts.length > 0 ? (
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <Card title="回ごとの1問あたりの時間" subtitle="下がっているほど速くなっている">
              <AttemptPaceChart data={attempts} />
            </Card>
          </div>
        ) : (
          <Card>
            <EmptyState title="まだ解いていません" hint="「この問題を始める」から計測できます" />
          </Card>
        )}

        <Card title="この練習問題の記録">
          <SessionList
            sessions={[...mine].sort((a, b) => b.started_at.localeCompare(a.started_at))}
          />
        </Card>
      </div>
    </>
