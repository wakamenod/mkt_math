...
export function CategoryDetailPage() {
  ...
  if (categories.isLoading || sets.isLoading || sessions.isLoading)
    return <Spinner />;
  ...
  const categorySets = (sets.data ?? []).filter(
  ...
  const setStats = new Map(
    byExerciseSet(categorySessions).map((s) => [s.exerciseSetId, s]),
  );
  ...
  return (
    <>
      ...
      {cov && (
        <div className="mt-3">
          <ProgressBar
            value={cov.totalSets === 0 ? 0 : cov.attemptedSets / cov.totalSets}
          />
        </div>
      )}
