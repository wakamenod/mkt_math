...
function session(p: {
  ...
  return {
    ...
    study_date: p.date,
    exercise_set: { id: p.setId, number: p.number, problem_count: p.problems, category: cat },
  }
}
...
describe('集計の基本', () => {
  it('空配列でゼロ除算しない', () => {
    ...
    expect(summarize([])).toMatchObject({ sessionCount: 0, accuracy: null, secondsPerProblem: null })
  })

  it('正答率の分母は problem_count_snapshot', () => {
    const s = [
      session({ setId: 'e1', number: 1, problems: 5, correct: 4, seconds: 300, date: '2026-09-14' }),
      session({ setId: 'e2', number: 2, problems: 5, correct: 3, seconds: 200, date: '2026-09-15' }),
    ]
  ...
  it('マスタの problem_count を変えても過去の正答率は動かない', () => {
    const s = session({ setId: 'e1', number: 1, problems: 5, correct: 4, seconds: 300, date: '2026-09-14' })
...
describe('日次系列', () => {
  const sessions = [
    session({ setId: 'e1', number: 1, problems: 5, correct: 5, seconds: 300, date: '2026-09-13' }),
    session({ setId: 'e2', number: 2, problems: 5, correct: 3, seconds: 600, date: '2026-09-15' }),
    session({ setId: 'e3', number: 3, problems: 5, correct: 2, seconds: 300, date: '2026-09-15' }),
  ]
...
describe('大分類別・練習問題別', () => {
  const sessions = [
    session({ setId: 'e1', number: 1, problems: 5, correct: 5, seconds: 100, date: '2026-09-13' }),
    session({ setId: 'g1', number: 1, problems: 4, correct: 1, seconds: 400, date: '2026-09-14', category: CAT2 }),
  ]
  ...
  it('累計問題数が少ないセットは苦手ランキングから除外される', () => {
    const tiny = [session({ setId: 'e9', number: 9, problems: 1, correct: 0, seconds: 60, date: '2026-09-15' })]
...
describe('反復による改善', () => {
  const sessions = [
    session({ setId: 'e6', number: 6, problems: 10, correct: 4, seconds: 600, date: '2026-09-10' }),
    session({ setId: 'e6', number: 6, problems: 10, correct: 9, seconds: 300, date: '2026-09-15' }),
  ]
  ...
  it('1回しか解いていないセットは対象外', () => {
    const once = [session({ setId: 'e1', number: 1, problems: 5, correct: 5, seconds: 100, date: '2026-09-13' })]
...
describe('ヒートマップ', () => {
  ...
  it('学習時間に応じて濃度が上がる', () => {
    const h = heatmap(
      [
        session({ setId: 'e1', number: 1, problems: 5, correct: 5, seconds: 60, date: '2026-09-14' }),
        session({ setId: 'e2', number: 2, problems: 5, correct: 5, seconds: 3000, date: '2026-09-15' }),
      ],
      4,
      '2026-09-15',
    )
