...
describe('computeStreak', () => {
  ...
  it('2日以上空いたら途切れている', () => {
    const r = computeStreak(['2026-09-10', '2026-09-11', '2026-09-12'], '2026-09-15')
    expect(r.current).toBe(0)
    expect(r.longest).toBe(3)
  })

  it('同じ日に複数セッションあっても1日として数える', () => {
    const r = computeStreak(['2026-09-14', '2026-09-14', '2026-09-15'], '2026-09-15')
    expect(r.current).toBe(2)
    expect(r.totalDays).toBe(2)
  })

  it('最長記録は過去の連続にも反応する', () => {
    const dates = ['2026-09-01', '2026-09-02', '2026-09-03', '2026-09-04', '2026-09-14', '2026-09-15']
  ...
  it('月をまたいでも連続として数える', () => {
    const r = computeStreak(['2026-08-30', '2026-08-31', '2026-09-01'], '2026-09-01')
    expect(r.current).toBe(3)
  })

  it('1日だけの学習', () => {
    const r = computeStreak(['2026-09-15'], '2026-09-15')
    expect(r).toMatchObject({ current: 1, longest: 1, studiedToday: true, totalDays: 1 })
  })
})
