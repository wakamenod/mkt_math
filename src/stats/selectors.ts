import { addDaysIso, dayOfWeekIso, rangeIso, todayJst, type IsoDate } from '../lib/date'
...
export function dailySeries(sessions: SessionWithSet[]): DailyPoint[] {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    points[i].accuracyMA7 =
      denom === 0 ? null : window.reduce((a, p) => a + p.correctCount, 0) / denom
  }
  return points
}

/** 直近 n 日ぶんを、学習していない日も 0 で埋めて返す（棒グラフ用）。 */
export function recentDaily(sessions: SessionWithSet[], days: number, today: IsoDate = todayJst()) {
...
export function accuracyByCategoryOverTime(sessions: SessionWithSet[]) {
  const categories = byCategory(sessions).map((c) => ({ id: c.categoryId, name: c.categoryName }))
  const byDate = groupBy(sessions, (s) => s.study_date)
  const rows = [...byDate.entries()]
    ...
    .map(([date, list]) => {
      ...
      for (const c of categories) {
        row[c.name] = accuracy(list.filter((s) => s.exercise_set.category.id === c.id))
      }
      return row
    })
    ...
    .map(([exerciseSetId, list]) => {
      ...
      return {
        ...
        lastStudiedAt: list.reduce<string | null>(
          (latest, s) => (latest === null || s.started_at > latest ? s.started_at : latest),
          null,
        ),
        ...summarize(list),
      }
    })
    .sort((a, b) => a.categoryName.localeCompare(b.categoryName) || a.number - b.number)
}
 ...
 */
export function weakestSets(sessions: SessionWithSet[], limit = 10, minProblems = 3) {
...
/** ある練習問題の、1回目・2回目…の推移。 */
export function attemptSeries(sessions: SessionWithSet[], exerciseSetId: string): AttemptPoint[] {
  return sessions
    ...
    .map((s, i) => ({
      attempt: i + 1,
      date: s.study_date,
      accuracy: s.problem_count_snapshot === 0 ? null : s.correct_count / s.problem_count_snapshot,
      secondsPerProblem:
        s.problem_count_snapshot === 0 ? null : s.duration_seconds / s.problem_count_snapshot,
